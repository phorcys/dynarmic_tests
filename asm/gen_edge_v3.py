...
def write_test(name, code, expected_regs):
    """写入测试文件"""
...
def gen_all_tests():
    ...
    # =========================================
    # 1. 位移指令边界条件测试 (正确的ARM语义)
    # ARM64: LSL/LSR/ASR (register) 自动掩码 shift amount
    # 32位: shift & 31, 64位: shift & 63
    # =========================================
    
    # LSL 32位: shift=32 -> shift&31=0 -> 结果=原值
    write_test('lsl_reg_mask_32',
        '''
    mov w0, #0x5678
...
''',
    ...
    # LSL 32位: shift=33 -> shift&31=1
    write_test('lsl_reg_mask_33',
        '''
    mov w0, #0x5678
    movk w0, #0x1234, lsl #16   // w0 = 0x12345678
    mov w1, #33
...
''',
    ...
    # LSR 32位: shift=32 -> shift&31=0 -> 结果=原值
    write_test('lsr_reg_mask_32',
        '''
    mov w0, #0x5678
    movk w0, #0x1234, lsl #16   // w0 = 0x12345678
    mov w1, #32
    lsr w2, w0, w1              // w2 = w0 >> (32 & 31) = w0 >> 0 = w0
''',
        {'X2': '0x0000000012345678'}
    )
    count += 1
    
    # ASR 32位: shift=32 -> shift&31=0 -> 结果=原值 (正数)
    write_test('asr_reg_mask_pos',
        '''
    mov w0, #0x5678
...
''',
    ...
    # ASR 32位: shift=32 -> shift&31=0 -> 结果=原值 (负数)
    write_test('asr_reg_mask_neg',
        '''
    mov w0, #0xFFFF
...
''',
    ...
    # LSL 64位: shift=64 -> shift&63=0 -> 结果=原值
    write_test('lsl64_reg_mask_64',
        '''
    mov x0, #0xDEF0
...
''',
    ...
    # LSL 64位: shift=65 -> shift&63=1
    write_test('lsl64_reg_mask_65',
        '''
    mov x0, #0xDEF0
    movk x0, #0x9ABC, lsl #16
    movk x0, #0x5678, lsl #32
...
''',
    ...
    mov w1, #0x12345678
    bfi w1, w0, #8, #16   // insert bits [15:0] of w0 into w1 at position 8, width 16
                          // w0[15:0] = 0xFF00
                          // w1 = 0x12FF0078
''',
