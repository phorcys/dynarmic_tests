#!/usr/bin/env python3
"""
Generate comprehensive A64 tests - Fixed version 3
"""
import os
...
""")
    ...
    # 扩展寄存器 - 0x80 作为 8/16/32 位都是有符号扩展为负数
...
""")
    ...
    # 标志位测试 - Z=bit30, C=bit29, N=bit31, V=bit28
...
""")
    ...
    # EON: ~(0xF0 ^ 0x0F) = ~0xFF = 0xFFFFFFFFFFFFFF00
    gen_test("eon_reg", {
        "Match": "All",
...
""")
    ...
    # sbfx: 0x78 的 bit 3 = 0，所以正数扩展后是 0x8
    gen_test("sbfx_sign", {
        "Match": "All",
...
""")
    ...
    # smull: w1 = 0xFFFF (有符号 -1), w2 = 1, 结果 = -1
...
""")
    ...
    for cond, exp in conditions:
        gen_test(f"csel_{cond}", {
            "Match": "All",
            "RegData": {"X0": f"0x00000000000000{exp}"}
        }, f"""
.text
.global _start
_start:
...
""")
    ...
    # CLS(2): 2 = 0b10, 前导 0 = 62, CLS = 62 - 1 = 61 = 0x3D
    gen_test("cls_basic", {
        "Match": "All",
        "RegData": {"X0": "0x000000000000003D"}
    }, """
.text
.global _start
_start:
...
""")
    ...
    # REV16: 交换 16 位半字内的字节
    # 0x0078_5634_0012 -> 每个半字内字节交换 -> 0x0078_3412_0056_7800
    gen_test("rev16_basic", {
        "Match": "All",
        "RegData": {"X0": "0x0078341200567800"}
    }, """
.text
.global _start
_start:
    mov x0, #0x0012
    movk x0, #0x5634, lsl #16
    movk x0, #0x78, lsl #32
    rev16 x0, x0
    brk #0
""")
...
_start:
    mov x0, #0x0000
    movk x0, #0x5600, lsl #16
    movk x0, #0x3412, lsl #32
    movk x0, #0x7800, lsl #48
    rev64 x0, x0
    brk #0
""")
...
""")
    ...
    # fcmp 2.0, 1.0: 2.0 > 1.0, N=0, C=1 -> 0x20000000
    gen_test("fcmp_gt", {
        "Match": "All",
...
""")
    ...
    # mls: v0 = 1 - (2 * 3) = 1 - 6 = -5 = 0xFFFFFFFB
    gen_test("vmls_4s", {
        "Match": "All",
        "RegData": {"X0": "0xFFFFFFFBFFFFFFFB"}
    }, """
.text
.global _start
_start:
