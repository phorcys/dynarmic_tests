...
// inline-asm SBC borrow producer -> page align -> header/payload object init.
...
static __attribute__((noinline)) u32 build_sbc_offset(void) {
    register u32 a __asm__("r0") = 0x0000F800u;
    register u32 b __asm__("r1") = 0x00000001u;
...
int test_main(void) {
    ...
    const u32 offset = build_sbc_offset();
    ...
    TEST_ASSERT(offset == 0x0000F7FEu);
    TEST_ASSERT(producer == arena_base + 0x0000F7FEu);
