...
// inline-asm SSAT producer -> page align -> header/payload object init.
...
static __attribute__((noinline)) u32 build_ssat_offset(void) {
    register s32 input __asm__("r0") = 0x0000FC77;
    register u32 out __asm__("r1");
    __asm__ volatile(
        "ssat %[out], #16, %[in]\n"
        : [out] "=r"(out)
        : [in] "r"(input)
...
int test_main(void) {
    ...
    const u32 offset = build_ssat_offset();
    ...
    TEST_ASSERT(offset == 0x00007FFFu);
    TEST_ASSERT(producer == arena_base + 0x00007FFFu);
    TEST_ASSERT(header_base == arena_base + 0x00008000u);
    TEST_ASSERT(payload_base == arena_base + 0x00018000u);
